...
export default function SearchBar({ onClick, className = "" }: SearchBarProps) {
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`group flex items-center gap-2 px-3 py-1.5 rounded-xl border border-slate-200/80 dark:border-slate-800 bg-slate-50/80 dark:bg-slate-950/60 text-slate-400 dark:text-slate-500 hover:border-cyan-500/40 hover:bg-white dark:hover:bg-slate-900 transition-all cursor-pointer select-none ${className}`}
    >
      <Search
        size={14}
        className="group-hover:text-blue-600 dark:group-hover:text-cyan-400 transition-colors shrink-0"
      />
