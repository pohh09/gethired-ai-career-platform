...
export default function ResumeWorkspace() {
  ...
  const handleGenerateResume = async () => {
    ...
    try {
      const data = await aiService.generateResume({ name: targetRole ? `Candidate (${targetRole})` : "Candidate", skills: resumeText ? [resumeText] : ["React", "TypeScript", "Node.js"] });
    ...
    <div className="space-y-6">
      ...
      <div className="p-6 sm:p-8 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm space-y-5">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 dark:border-slate-800 pb-4">
          ...
          <label className="inline-flex items-center justify-center gap-2 px-4 py-2 rounded-2xl bg-indigo-600 hover:bg-indigo-700 text-white font-extrabold text-xs cursor-pointer shadow-md shadow-indigo-600/20 transition-all shrink-0">
            <Upload size={14} />
        ...
        <div className="space-y-4">
          ...
          <Input
            ...
            placeholder="e.g. Built React analytics dashboard."
          />
        </div>
        ...
        <div className="p-6 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xl space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="md:col-span-2 p-4 rounded-2xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 space-y-2">
              ...
              <p className="text-xs text-slate-600 dark:text-slate-300 leading-relaxed">{auditData.summary}</p>
            </div>
          </div>
